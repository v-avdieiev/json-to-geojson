...
app.get("/api/", function (req, res) {
  ...
  superAgent.get(req.query.endpoint).end(function (err, apiRes) {
    if (err) {
      res.status(500);
      return res.end("" + err);
    }
    try {
      ...
      res.status(500);
      return res.end(
        `Please ensure you're connecting to a JSON API and that it returns objects with 'lat' and 'long' properties`,
      );
    }
  });
});
